using UnityEngine;
using System.Collections;
using System.IO;
using System.Xml;

public class TramController : MonoBehaviour {

    public bool debugMe = false;
    public string routeName = "BuryOut";

    //Move variables
    public GameObject[] tramStops;
    public int startStopId = 0;
    private int previousStopId = 0;
    private int targetStopId = 0;
    private int totalNoStops;
    //public bool Outbound = true;
    private float percentageAlongPath = 1;
    public float timeToReachNextStation;
    private int currentTrip = 0;

    // Use this for initialization
    void Start () {
        transform.position = tramStops[startStopId].transform.position;
        totalNoStops = tramStops.Length;
        previousStopId = startStopId;
        targetStopId = previousStopId;
       
    }
	
	// Update is called once per frame
	void Update () {
        if(!XmlScheduleData.loadingData)
        {
            if (TimeInfo.currentTime >= XmlScheduleData.routes[routeName].trips[0].scheduledStops[0].stopTime)
            {
                if (percentageAlongPath >= 1)
                {
                    ReachedStation();

                }
                else
                {
                    MoveToNextStation();
                }
            }
            else
            {
                if(debugMe)
                {
                    Debug.Log(XmlScheduleData.routes[routeName].trips[currentTrip].scheduledStops[0].stopTime);
                }
                
            }
            




            //int stopTime = XmlScheduleData.routes[routeName].trips[0].scheduledStops[targetStopId].stopTime;
            //Debug.Log(stopTime);
            //if (TimeInfo.currentTime >= stopTime)
            //{
            //    transform.position = tramStops[targetStopId].transform.position;
            //    previousStopId = targetStopId;
            //    targetStopId += 1;
            //}

        }
        
    }

    public void MoveToNextStation()
    {

        percentageAlongPath += (Time.deltaTime / timeToReachNextStation);

        transform.position = Vector3.Lerp(tramStops[previousStopId].transform.position, tramStops[targetStopId].transform.position, percentageAlongPath);

    }

    void ReachedStation()
    {
        previousStopId = targetStopId;
        ChangeTargetStation();

        int nextStopTime = XmlScheduleData.routes[routeName].trips[currentTrip].scheduledStops[targetStopId].stopTime;
        timeToReachNextStation = nextStopTime - TimeInfo.currentTime;
        percentageAlongPath = 0;


        



    }

    public void ChangeTargetStation()
    {
        
        if (targetStopId == XmlScheduleData.routes[routeName].trips[currentTrip].scheduledStops.Count - 1) //end of line
        {
            targetStopId = 1;
            previousStopId = 0;
            currentTrip += 1;
            transform.position = tramStops[previousStopId].transform.position;
        }
        //else if (targetStopId == 0) //back at the start
        //{
        //    //Debug.Log("You are back home!");
        //    Outbound = true;
        //    targetStopId += 1;
        //}
        //else if (Outbound) //moving to end
        //{
        //    targetStopId += 1;
        //}
        //else if (!Outbound)//moving to start
        //{
        //    targetStopId -= 1;
        //}
        else
        {
            targetStopId += 1;
        }
    }


}
